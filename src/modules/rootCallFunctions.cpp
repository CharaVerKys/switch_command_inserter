...
void showHelp(){
	...
	Возможно 2 варианта использования: настройка всей сети (scan identify) и работа с набором адресов (script)
	script:
		в configs/perModel_doCommands.conf выставить регулярку модели совпадаю с script (script|sc|.*) и команды которые хотите выполнить
		зайти в configs/scriptIp.list и вставить туда свой набор ip для обработки
		запустить с глаголом script или scripTELN (можно по очереди чтобы одновременно обработать оба протокола)
		запустить с глаголом commit (-y)
		посмотреть результат работы в логах (основные логи находятся в корне)
	scan identify:
		в configs/network.conf установить необходимую подсеть, логин/пароль
		запустить с глаголом scan
		придумать набор необходимых условий для определения модели коммутатора (это может быть например expect cisco 2950 команды show run) То есть можно токно настроить это. Записать конфигурацию в файл configs/perCommands_findModel.conf Можно использовать уже сделанную конфигурацию для коммутаторов с github проекта
		запустить с глаголом identify
		в configs/perModel_doCommands.conf выставить регулярку модели, где то что было выставленно в perCommands_findModel.conf является полным именем коммутатора
		запустить с глаголом commit
		посмотреть результат работы в логах (основные логи находятся в корне)

	Очень важные моменты при работе с программой:
		нельзя отправлять ? в некоторых коммутаторах ( в частности cisco ) = приводит к тому что программа не сможет корректно считать следующий вывод (точнее можно, но если это последняя команда которую вы хотите отправить)
		в прошивке cisco для show run и прочих с --More-- требуется ввести term le 0, иначе будет получен не полный вывод команды
		таймаут подключения - 2 минуты (меняется в коде) в течении которых программа не может считать ответ от удалённого хоста (для каждого хоста отдельный)
		linksys коммутаторы - программно странно реализован интерфейс ssh и к нему из программы подключиться невозможно (как и некоторые другие старые прошивки)

	В случае нахождения багов пишите мне charaverk@gmail.com /TG: t.me/Chara_VerKys
)";
std::cout << help;
	}
