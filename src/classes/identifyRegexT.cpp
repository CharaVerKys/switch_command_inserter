    ...
    try
    {
        ...
        asio::async_read(_socket, _read_buffer, asio::transfer_at_least(1), [this, self](const asio::error_code &error, size_t bytes_transferred)
            ...
            if (!error)
            {
                ...
                if (std::regex_search(_last_read.str(),_regex_login))
                {
                ++_login_iteration;
                   one_it_authent_try();
                }
                else if(std::regex_search(_last_read.str(),_regex_end_of_read))
                {
...
void IdentifyTELNET::exec_com()
{
    ...
    try
    {
        ...
        if (!error) {
            ...
            start_timer();
            read_from_host_init(_finding_commands.at(_cover_iteration).second.at(_command_iteration).send_to_step.c_str());
